...
import { BrandApi } from 'src/app/Model/brands';
import { AddToCartModel } from 'src/app/Model/cart';
import { Brand } from 'src/app/Model/homePage';
...
export class ProductDesktopComponent {
  ...
  private setproductimage(productDetils: ProductData) {
    if (productDetils.product.product_images.length != 0) {
      this.productDetils.product.product_images.map((image: ProductImage) => {
        this.productImage.push(image.full_product_image);
      });
    } else {
      this.productImage = ['assets/product/images/noimage.jpg'];
    }
  }
  // product api
